...
CKEDITOR.dialog.add('link', function (editor) {
    ...
    var loadGalleryFrame = function (elementID, galleryID) {
        if (jQueryKam("#" + elementID + " iframe").attr("src") == null) {
            jQueryKam("#" + elementID + " iframe").attr("src", KAM_ROOT + "files?type=" + galleryID + "&extra_class=in_cke_editor&single_mode=1");
        } else {
            jQueryKam("#" + elementID + " iframe")
                .contents()
                .find("div.elfinder-workzone div.ui-selected").removeClass("ui-selected")
                .find("div.ui-state-hover").removeClass("ui-state-hover");
        }
    }

    // Handles the event when the "Type" selection box is changed.
    var linkTypeChanged = function (e) {
        var dialog = this.getDialog(),
            partIds = [ 'urlOptions', 'anchorOptions', 'emailOptions', 'linkOptions', 'imageOptions', 'fileOptions' ],
        ...
        for (var i = 0; i < partIds.length; i++) {
            ...
            if (element.id == "linkOptions") {
                var URL = "";
                if (e.data.value == "link") {
                    URL = KAM_ROOT + "tree?multi_langs=1"
                    if (linkID) {
                        URL += "&node=" + linkID;
                    }
                }
                jQueryKam("#" + element.domId + " iframe").attr("src", URL);
            }

            if (element.id == "imageOptions" && e.data.value == "image") {
                loadGalleryFrame(element.domId, 1);
            }

            if (element.id == "fileOptions" && e.data.value == "file") {
                loadGalleryFrame(element.domId, 2);
            }
    ...
    var linkRegex  = /^kameleon:link\((.*)\)$/,
        imageRegex = /^kameleon:image\((.*)\)$/,
        fileRegex  = /^kameleon:file\((.*)\)$/;
    ...
    var parseLink = function (editor, element) {
        ...
        if (!retval.type) {
            ...
            else if (linkMatch = href.match(linkRegex)) {
                retval.type = "link";
                retval.link = {};
                var parts = linkMatch[1].split(",");
                linkID = parts[0];
                retval.link.variables = parts[1] || "";
            }
            else if (imageMatch = href.match(imageRegex)) {
                retval.type = "image";
                retval.image = {};
                var parts = imageMatch[1].split(",");
                fileID = parts[0];
            }
                        ...
                        children : [
                            {
                                type : "html",
                                id : "linkTree",
                                html : '<div style="height: 350px; border: 1px solid #bfbfbf;">'
                                     + '<iframe frameborder="0" style="padding: 0; border: 0; width: 100%; height: 100%;"></iframe>'
                                     + '</div>'
                            },
                            {
                                type : "text",
        ...
        onOk: function () {
            ...
            switch (data.type || 'url') {
                case "link" :
                    ...
                    break;
                case "image":
//                    attributes[ 'data-cke-saved-href' ] = "kameleon:image(" + fileID + ")";
                    attributes[ 'data-cke-saved-href' ] = fileID;
                    attributes[ 'fancybox' ] = 1;
                    break;
                case "file":
//                    attributes[ 'data-cke-saved-href' ] = "kameleon:file(" + fileID + ")";
                    attributes[ 'data-cke-saved-href' ] = fileID;
                    break;
                case 'url':
...
//                            text = text.split('/').pop().slice(0, -1);
        ...
        onLoad: function () {
            ...
            jQueryKam("body").on("tree_node_selected", function (e, metadata) {
                linkID = metadata.id;
            });

            jQueryKam("body").on("picker_file_selected", function (e, file) {
                fileID = file.url.replace(KAM_ROOT, "");
            });
        },
