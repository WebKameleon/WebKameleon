...
//				if (call_user_func($handler,$cmd,$result,$args,$this)) {
                if ($handler($cmd, $result, $args, $this)) {
	...
	protected function mkdir($args) {
		$target = $args['target'];
		$name   = Kameleon::str_to_url($args['name']);
	...
	**/
	...
	protected function upload($args) {
		...
		foreach ($files['name'] as $i => $name) {
			...
			if (! $_target || ($file = $volume->upload($fp, $_target, Kameleon::str_to_url($name), $tmpname)) === false) {
